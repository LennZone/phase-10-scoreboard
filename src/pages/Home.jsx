...
function GameCard({ game, uid, onJoin, onDelete }) {
  ...
  const statusLabel = {
    lobby: { text: t('gameCard.lobby'), color: 'text-amber-400' },
    active: { text: t('gameCard.active'), color: 'text-emerald-400' },
    finished: { text: t('gameCard.finished'), color: 'text-slate-500' },
  };
  const statusInfo = statusLabel[game.status] ?? statusLabel.finished;

  const date =
    game.createdAt?.toDate?.()?.toLocaleDateString(i18n.resolvedLanguage, {
      day: '2-digit',
      month: '2-digit',
      year: 'numeric',
    }) ?? '–';
      ...
      <div className="flex items-start justify-between gap-2">
        <div className="min-w-0">
          <div className="flex flex-wrap items-center gap-2">
            <span className="font-mono text-xs tracking-widest text-slate-500">{game.id}</span>
            <span className={`text-xs font-medium ${statusInfo.color}`}>{statusInfo.text}</span>
            {isWinner && (
              <span className="text-xs font-medium text-amber-400">{t('gameCard.winner')}</span>
            )}
          </div>
          <div className="mt-0.5 text-xs text-slate-500">
            {date} · {t('gameCard.players', { count: playerCount })}
            {isHost && <span className="ml-1 text-slate-600">{t('gameCard.youHost')}</span>}
          </div>
        </div>
        {myState && (
          <div className="shrink-0 text-right">
            ...
            <div className="text-xs text-slate-500">
              {myState.totalScore} {t('scoreCard.pts')}
            </div>
          </div>
        )}
      </div>
...
export default function Home() {
  ...
  const { user, isGoogleUser, signInWithGoogle, signOutUser, createGame, deleteGame } =
    useGame(null);
  ...
  return (
    <div className="flex min-h-screen flex-col items-center justify-center p-6">
      ...
      <div className="mb-10 text-center">
        <h1 className="mb-1 text-6xl font-bold leading-none tracking-tight text-white">
      ...
      <div className="w-full max-w-xs space-y-3">
        ...
        {/* Google Auth */}
        {isGoogleUser ? (
          <div className="glass-card flex h-12 items-center justify-between rounded-xl px-4">
          ...
          <button
            ...
            className="inline-flex h-12 w-full items-center justify-center gap-2.5 rounded-lg border border-white/10 bg-white/90 px-6 text-sm font-medium text-slate-800 shadow-sm transition-all hover:bg-white active:scale-95 disabled:opacity-50"
          >
            <GoogleIcon />
        ...
        {isGoogleUser && (
          <div className="space-y-4 pt-4">
            {historyLoading ? (
              <p className="py-2 text-center text-sm text-slate-500">{t('game.loading')}</p>
            ) : (
              <>
                ...
                {games.length === 0 && (
                  <p className="py-2 text-center text-sm text-slate-500">{t('home.noGames')}</p>
                )}
              </>
            )}
          </div>
        ...
        {!isGoogleUser && (
          <p className="pt-2 text-center text-xs text-slate-500">{t('home.saveHint')}</p>
        )}
      </div>
      ...
      >
        ...
        <p className="mb-4 text-xs text-slate-500">{t('home.delete.warning')}</p>
